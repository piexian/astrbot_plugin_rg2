...
### 🐛 问题修复
- **状态文本配置**: 修复 `game_status` 分类未在游戏状态标题中生效的问题。
- **角色查询优化**: 复用中弹和随机走火流程中的可禁言判断结果，避免重复查询群成员角色。

### 🧹 代码整理
- **格式化对齐**: 使用 `ruff format` 对代码格式进行统一。
- **版本同步**: 同步 `metadata.yaml`、README 徽章与运行时兜底版本号。

## [1.2.6] - 2026-02-21

### 📝 元数据更新
- **新增 AstrBot 版本要求**: 添加 `astrbot_version: ">=3.5.0"` 声明最低兼容版本
- **新增支持平台声明**: 添加 `support_platforms: [aiocqhttp]`，明确插件仅支持 OneBot v11 协议（如 NapCat、go-cqhttp 等）
- 适配 AstrBot PR #5235 的插件元数据规范

## [1.2.5] - 2026-01-22

### ✨ 新增功能
- **游戏规则自定义**: 
  - 新增 `max_bullet_count`：支持自定义最大子弹数及弹膛总数。
  - 新增 `fixed_bullet_count`：支持设置固定装弹数量，不使用随机。
  - 新增 `no_full_chamber`：开启后随机装填不会出现满膛情况。
  - 新增 `hide_bullet_count`：支持在装填消息中隐藏具体子弹数（显示为“?”）。
  - 新增 `end_on_full_rotation`：开启后若弹膛内全是实弹则立即结束游戏。

### 🔧 逻辑优化
- **射击逻辑**: 引入 `shot_count` 射击计数器，优化弹膛轮转和自动结束判定。
- **说明优化**: 精简了部分指令说明文字并优化了类型注解。
